...
namespace Engine
{
    public class Player : LivingCreature
    {
        ...
        public void RemoveQuestCompletionItems(Quest quest)
        {
            foreach (QuestCompletionItem qci in quest.QuestCompletionItems)
            {
                //Suubtract the quantity from the player's inventory that was needed to complete the quest
        ...
        public void RemoveItemFromInventory(Item itemToRemove, int quantity = 1)
        {
            ...
            if (item == null)
            {
                //The item is not in the player'sinventory, so ignore it.
        ...
        {
            //Add healing amount to theplayer's current hit points
            CurrentHitPoints = (CurrentHitPoints + potion.AmountToHeal);

            //current hit points can't exceed player's MaximumHitPOints
        ...
        public string ToXmlString()
        {
            ...
            // Create an "InventoryItems" child node to hold each InventoryItem node
            XmlNode inventoryItems = playerData.CreateElement("InventoryItems");
            player.AppendChild(inventoryItems);

            // Create an "InventoryItems" node for each item in the player's inventory
